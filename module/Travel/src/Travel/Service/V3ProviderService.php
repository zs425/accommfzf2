...
class V3ProviderService extends AbstractService
{
    ...
    public function isValidService()
    {
        $request = '<?xml version="1.0"?>
                <CABS_ProviderOptIn_RQ xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xsd="http://www.w3.org/2001/XMLSchema" xmlns="http://www.v3leisure.com/Schemas/CABS/1.0/CABS_ProviderOptIn_RQ.xsd">
                    <Channels>
                        <DistributionChannel id="' . $this->getAuth()->channelid . '" key="' . $this->getAuth()->channelkey . '" xmlns="http://www.v3leisure.com/Schemas/CABS/1.0/CABS_Common.xsd" />
                </Channels>
    ...
    public function getProviderOptIns($values = null)
    {
        ...
        $request->Channels->DistributionChannel = array("id" => $this->getAuth()->channelid, "key" => $this->getAuth()->channelkey);
        $request->Providers                     = "";
    ...
    public function getProviderSearchs($array)
    {
        ...
        $request                                         = new \stdClass();
        $request->Channels->CO_DistributionChannelRQType = array("id" => $this->_auth->channelid, "key" => $this->_auth->channelkey);
        $request->Providers                              = "";
    ...
    {
        $request                                         = new \stdClass();
        $request->Channels->DistributionChannelRQ        = array("id" => $this->getAuth()->channelid, "key" => $this->getAuth()->channelkey);
    ...
    public function getCalendarSearch($values)
    {
        ...
        $request->Providers                     = new \stdClass();
        ...
        //Cache Mode
        if (isset($values["rdoAuto"]) && $values["rdoAuto"] == "on") //Auto
        {
            $request->Query = array("cache" => "Auto");
        }

        if (isset($values["rdoOn"]) && $values["rdoOn"] == "on") //On
        {
            $request->Query = array("cache" => "On");
        }

        if (isset($values["rdoOff"]) && $values["rdoOff"] == "on") //Off
        {
            $request->Query = array("cache" => "Off");
        }

        //Show Product Info in results
        if (isset($values["chkShowProduct"]) && $values["chkShowProduct"] == "on") {
        ...
        try {
            ...
            if (count($request->Providers->Provider) > 10) {
                ...
                if (isset($res->Status->Success) && $res->Status->Success) {
                    if (isset($res->Channels->Channel->Providers->Provider)) {
                        return $res->Channels->Channel->Providers->Provider;
                    }
                    return array();
                }
