...
function matchDestinations() {
	roamfreeId = $('input:radio[name=roamfeeId]:checked').val();
	destinationId = $('input:radio[name=destinationId]:checked').val();
	
	if(typeof(roamfreeId) === "undefined" || typeof(destinationId) === "undefined" || roamfreeId == 0 || destinationId == 0) {
		html = '<div class="alert alert-warning fade in">';
		html += '<button class="close" data-dismiss="alert">×</button>';
		html += '<i class="fa-fw fa fa-warning"></i>';
		html += 'Please select roamfree destination record and destination record.';
		html += '</div>';
		
		$('.match-div .alert-div').html(html);
		return;
	}
	
	$.ajax({
		type: "POST",
		url:"/admin/centraldb/manualmatchdestination/manual-match",
		data: "roamfreeId=" + roamfreeId + "&destinationId=" + destinationId,
	}).done(function(html) {
		html = '<div class="alert alert-success fade in">';
		html += '<button class="close" data-dismiss="alert">×</button>';
		html += '<i class="fa-fw fa fa-check"></i>';
		html += '<strong>Matched successfully.</strong>';
		html += '</div>';
		$('.match-div .alert-div').html(html);		
	});
